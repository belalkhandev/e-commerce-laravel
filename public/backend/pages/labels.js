...
function onPaginationDataLoad(page) {
	$.ajax({
		url:base_url + "/backend/getLabelsTableData?page="+page,
...
function onRefreshData() {
	$.ajax({
		url:base_url + "/backend/getLabelsTableData",
		success:function(data){
			$('#tp_datalist').html(data);
			onCheckAll();
		}
...
function onSearch() {
	...
	$.ajax({
		url: base_url + "/backend/getLabelsTableData?search="+search,
		success:function(data){
			$('#tp_datalist').html(data);
			onCheckAll();
		}
...
function onFormPanel() {
    resetForm("DataEntry_formId");
	RecordId = '';
	$("#color").colorpicker('setValue', '#88c136');
...
function onConfirmWhenAddEdit() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/saveLabelsData',
...
function onLoadEditData() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/getLabelsById',
		data: 'id='+RecordId,
		success: function (response) {
			var data = response;
			$("#RecordId").val(data.id);
			$("#title").val(data.title);
			$("#color").colorpicker('setValue', data.color);
...
function onConfirmDelete() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/deleteLabels',
		data: 'id='+RecordId,
		success: function (response) {
			var msgType = response.msgType;
			var msg = response.msg;
...
function onConfirmBulkAction() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/backend/bulkActionLabels',
